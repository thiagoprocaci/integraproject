 ...
 * @version 1.0
 */
public class PageRedirectEnabledAuthenticationProcessingFilter extends
		AuthenticationProcessingFilter {

	private PageRedirectDefinitionSource pageRedirectDefinitionSource;

	/**
	 * <b>Description copied from {@link AbstractProcessingFilter#getDefaultTargetUrl()}</b>
	 * Supplies the default target Url that will be used if no saved request is
	 * found or the alwaysUseDefaultTargetUrl propert is set to true. Override
	 * this method of you want to provide a customized default Url (for example
	 * if you want different Urls depending on the authorities of the user who
	 * has just logged in).
	 */
	public String getDefaultTargetUrl() {

		Authentication authResult = SecurityContextHolder.getContext()
				.getAuthentication();
		...
		if (definition != null) {
			...
			if (logger.isDebugEnabled()) {
				logger
						.debug("PageRedirectDefinition not found for authentication :"
								+ authResult);
			}
		}
