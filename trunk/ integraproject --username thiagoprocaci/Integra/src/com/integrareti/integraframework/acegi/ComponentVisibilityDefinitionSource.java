...
public class ComponentVisibilityDefinitionSource {
	...
	/**
	 * Return the Id's of the ZK components to hide based on the {@link GrantedAuthority}[]
	 * Privided
	 * @param GrantedAuthority[] authorities
	 * @return List of Id's of the components
	 */
	public List<String> getComponentsToHide(
		...
		for (Iterator<ComponentVisibilityDefinition> iter = getDefinitionsList().iterator(); iter.hasNext();) {
			ComponentVisibilityDefinition definition = (ComponentVisibilityDefinition) iter
					.next();
