...
public class PageRedirectDefinitionSource {
	...
	private static final Log logger = LogFactory
			.getLog(PageRedirectDefinitionSource.class);

	private List<PageRedirectDefinition> definitionsList;
	
	/**
	 * Returns the {@link PageRedirectDefinition} based on the given {@link GrantedAuthority}[]
	 * @param authorities
	 * @return
	 */
	public PageRedirectDefinition getDefinitionFor(
		...
		for (Iterator<PageRedirectDefinition> iter = getDefinitionsList().iterator(); iter.hasNext();) {
			PageRedirectDefinition definition = (PageRedirectDefinition) iter
					.next();
			if (roleList.contains(definition.getRoleName())) {
				logger.debug("First matched PageRedirectDefinition :"
						+ definition);
				return definition;
			}
		}
